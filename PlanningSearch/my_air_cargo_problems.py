            ...
            for cargo in self.cargos: 
                for plane in self.planes:
                    for port in self.airports:
                        ...
                        effect_neg = [expr('At({}, {})'.format(cargo, port))]
                        
                        load_expr = expr('Load({}, {}, {})'.format(cargo, plane, port))
                        
                        load = Action(load_expr, [precond_pos, precond_neg], [effect_pos, effect_neg])
            ...
            for cargo in self.cargos: 
                for plane in self.planes:
                    for port in self.airports:
                        ...
                        effect_pos = [expr('At({}, {})'.format(cargo, port))]
                        effect_neg = [expr('In({}, {})'.format(cargo, plane))]
                        
                        unload_expr = expr('Unload({}, {}, {})'.format(cargo, plane, port))
                        
                        unload = Action(unload_expr, [precond_pos, precond_neg], [effect_pos, effect_neg])
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    # break inital state in positve and negative fluents
    pos = [expr('At(C1, SFO)'),
           expr('At(C2, JFK)'),
           expr('At(C3, ATL)'),
           expr('At(P1, SFO)'),
